...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!(await user.correctPassword(password, user.password))) {
      return next(new AppError("Password not correct for provided email", 401));
    }
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return next(new AppError("There was an error in sending a mail", 500));
    }
  } catch {
    next(err);
  }
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    next(new AppError("Something went wrong, Could not reset password", 500));
  }
};
